...
namespace Aresv2
{
    public partial class ucStokGruplari : UserControl
    {
        ...
        public clsTablolar.Stok.csStokGrupV2 YeniGruplama;
        ...
        private void btnGrupSec_Click(object sender, EventArgs e)
        {
            try
            {
                using (Stok.frmStokGruplari frm = new Stok.frmStokGruplari(Stok.frmStokGruplari.NasilAcsin.AramaIcin))
                {
                    ...
                    {
                        frm.SeciliStokGruplari = new List<Stok.frmStokGruplari.SeciliStokGrubu>();
                        foreach (var item in YeniGruplama.dt.AsEnumerable())
                        {
                            if (item.RowState != DataRowState.Deleted)
                                frm.SeciliStokGruplari.Add(new Stok.frmStokGruplari.SeciliStokGrubu() { StokGrupID = (int)item["StokGrupID"] });
                        }
                    }
        ...
        {
            //string aha = ;
            //MessageBox.Show(listBoxControl1.SelectedIndex.ToString() + "   " + aha +"   "+ listBoxControl1.SelectedValue.ToString());
            ...
            VerilerDegisti(null, null);
            //ButonlariAktifPasifYap(true);
            //YeniGruplama.dt.Rows[listBoxControl1.SelectedIndex].Delete();
        }
