...
namespace clsTablolar.BasitUretim
{
    public class csBasitUretimRecete : IDisposable
    {
        ...
        {
            using (cmd = new SqlCommand(@"select BasitUretimRecetesi.*, Stok.StokAdi, Stok.StokKodu from BasitUretimRecetesi 
inner join Stok on Stok.StokID = BasitUretimRecetesi.UretilenStokID

where BUReceteID = @BUReceteID", Baglanti, TrGenel))
            {
                ...
                using (dr = cmd.ExecuteReader())
                {
                    if (dr.Read())
                    {
                        ...
                        _StokAdi = dr["StokAdi"].ToString();
                        _StokKodu = dr["StokKodu"].ToString();
                    }
                }
            }
        }
